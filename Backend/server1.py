...
#this below function is used to handle the GET request from the client
    def do_GET(self):
        ...
        responce_message=f"Hello from server 1  You are the {NeuralHTTP.number_request}th visitor to this server."  #this will create a message that says hello world and the number of requests received by the server
        self.wfile.write(responce_message.encode())                      # Convert the string to bytes and send it to the client because wfile only accepts bytes so we encode it 
...
if __name__ == '__main__':
    server = HTTPServer(("localhost" , 8001), NeuralHTTP)                #this will create a server object that will listen to the localhost and port number
    print("Server is running at http://localhost:8001")          #this will print the message that server is running at localhost and port number
