...
#this below function is used to handle the GET request from the client
    def do_GET(self):
        ...
        NeuralHTTP.number_request += 1                          #this will increment the number of request by 1 every time a request is made to the server
        responce_message = f"Hello from server 2 You are the {NeuralHTTP.number_request}th visitor to this server."  #this will create a message that says hello world and the number of request that has been made to the server
        self.wfile.write(responce_message.encode())                      #this will encode the message and send it to the client because wfile is a file like object that is used to write the response to the client
...
if __name__ == '__main__':
    server = HTTPServer(("localhost" , 8002), NeuralHTTP)                #this will create a server object that will listen to the localhost and port number
    print("Server is running at http://localhost:8002")          #this will print the message that server is running at localhost and port number
